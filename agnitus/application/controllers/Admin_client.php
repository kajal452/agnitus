...
class Admin_client extends CI_Controller 
{
	...
	public function client_edit()
	{
		...
		if(!isset($id) || !is_numeric($id)){
			redirect(base_url('admin/client_list'));exit; 
		}
		...
		$edit_result 	= getAlldataById('generic_clients',$whr);
		...
		$data['onMap'] = 1;
		$data['main_content']	= 'admin/client/edit_client';
		$this->load->view('admin/template',$data);
	...
	public function edit_client()
	{
		...
		$client_wallet_balance 	=    $this->input->post('client_wallet_balance');
		
	
		$update_array = array(
            'client_wallet_balance'           	=> $client_wallet_balance,
		...
		$update_result = updaterow($update_array,'generic_clients',$whr);
   		...
   		if($update_result == 'true')
   		{
			...
			redirect(base_url('admin/client_list'));
    	}else{
	       	$this->session->set_flashdata('notice', $message);
	   		$data['main_content']	= 'admin/client/edit_client';
			redirect(base_url('admin_client/client_edit?id='.$id));
			$this->load->view('admin/template',$data);
    	}
	...
	public function inactive_row()
	{
    	...
    	$inactive = updaterow($data_array,'generic_clients',$whr);
    	if($inactive == 'true'){
